...
import random
import time
from matplotlib.pyplot import figure, xlim,ylim, xlabel, ylabel, plot, show
from sf import *
...
data = genfromtxt("grover_data.csv",delimiter=',').T
PO4 = data[6] #(uM) NO3 concentration
Mu_d = data[7]  #(d-1) Growth rate
...
#Initial values
K = 0.5 #(uM) Initial Kno3: half saturation constant for NO3
Mumax = 0.7 #(d-1) Maximum growth rate 
...
plot(PO4_high_res,Mu,linewidth=4, linestyle='--', color='teal')
xlabel('PO$_{4}^{-}$ ($\mu$M)')
ylabel('$\mathit{\mu}$ (d$^{-1}$)')
ylim(ymax=0.8)
xlim(xmax=5.2)
sf('crytomonas_combine')
