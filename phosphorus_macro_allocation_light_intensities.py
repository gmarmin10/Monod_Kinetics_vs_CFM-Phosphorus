...
from matplotlib.pyplot import figure, plot, scatter, xlim, ylim, show, rcParams, xlabel, ylabel, title, stackplot, xticks, yticks, tight_layout, legend,title
...
def Plight(I):
    ...
    I=I    #(umolE m-2 s-1) light intensity
    Pchl=Pmax*(1-exp(-OT*I)) #(C mol s-1 Chl mol-1) Carbohydrate fixation rate per chlorophyll (167-1)(193-25)
    
    #Nutrient======================
    PO4 = arange(0.00001,5+1e-10,0.01) #(umol L-1)
    Loop_array=arange(0,size(PO4),1)     #array for loops
    Numbertoarray=ones(size(PO4))            #(dimensionless) Number to array converter
    ...
    for i in Loop_array:
        ...
        Vp=aPO4*PO4[i]       #(mol P cell-1 s-1) phosphorus uptake per cell 
        ...
        cPf=float(cP)
        dPf=float(dP)
        print(aPf,bPf,cPf,dPf)
        DP=solver_3D(aPf,bPf,cPf,dPf)
        D[i]=DP.X
        #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    Prna_plot=Prna*Punit*Numbertoarray    #(ug P/ mgC) Phosphorus in constant part of RNA
    ...
    Cnstore = zeros(size(PO4))#Nstore*YcyanoC_N
...
Xlabel = 'PO$_{4}^{3-}$ ($\mu$mol L$^{-1}$)'
Xlabel= 'PO$_{4}^{3-}$ (\u03bcmol L$^{-1}$)'

xtix=arange(0,4.5,1)
ytix=arange(0,2.5,0.5)

figure(1)
plot(PO4_10,D_10*86400, color='#EAECCC')
plot(PO4_20,D_20*86400, color='#FEDA8B')
plot(PO4_40,D_40*86400, color='#FDB366')
plot(PO4_80,D_80*86400, color='#F67E4B')
plot(PO4_160,D_160*86400, color='#DD3D2D')
#plot(PO4_320,D_320*86400, color='#A50026')
xlabel(Xlabel, fontsize=18)
xticks(xtix,fontsize=16)
yticks(ytix,fontsize=16)
ylabel("$\mathit{\mu}$ (d$^{-1}$)", fontsize=18)
ylim(bottom=0,top=2)
xlim(right=4.5)
tight_layout()
handles=['10','20','40','80','160'] #,'320'
legend(handles)
title('Light ($\mu$mol m$^{-2}$ s$^{-1}$)')
sf('light_intensity_P_change')
...
# 
# figure(6)
# stackplot(PO4,Pconst_other_plot+Pdna_const_plot,Pthylakoid_plot,Prna_plot,Pstore_plot,colors=StackPlotColorsP2)
# title("P:C")
# xlabel(Xlabel)
# ylabel("P:C (mol mol$^{-1}$)")
# # figure(2)
# # stackplot(NO3,Nconst_protein_plot,Nphoto_plot,Nbiosynth_plot,Ndna_const_plot,Nrna_plot,Nchl_plot,colors=StackPlotColorsN)
# # xlabel(Xlabel)
# # ylabel("N:C (mol mol$^{-1}$)")
# # 
# figure(4)
# stackplot(PO4,Cessential_plot,Cconst_protein_plot,Cphoto_plot,Cbiosynth_plot,Cdna_const_plot,\
#           Crna_plot,Cchl_plot,CthylakoidPG_plot,Cnstore_plot,Cother_plot,colors=StackPlotColorsC)
