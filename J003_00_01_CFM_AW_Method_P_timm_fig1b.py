...
import random
import time
from matplotlib.pyplot import figure, xlim,ylim, xlabel, ylabel, plot, show, stackplot
from sf import *
...
I=120    #(umolE m-2 s-1) light intensity
...
def function(aPO4,Ynphoto_chl,PO4):
    #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    #Main calculation 199-21~
    #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    Vp=aPO4*PO4    #(mol P cell-1 s-1) phosphorus uptake per cell
...
#OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
data = genfromtxt("timmermans_data.csv",delimiter=',').T
PO4 = data[0] #(uM) PO4 concentration
...
# Initial values
aP = 0.1*0.05*Qc/86400  #aPO4 #(mol N cell-1 s-1 / umolN L-1) affinity of PO4 (initial value from d007_06_00)
aP= 6e-20
...
for i in repeat_times:
    ...
    while True:
        aP1 = aP + a_aP*random.uniform(-1,1)
        if aP1>0 and aP1<10e-20:
            break
    ...
    Mu1 = function(aP1,Yn1,PO4)*86400
...
print('Pbest=', Pbest,', aPbest=', aPbest, ', Ynbest=', Ynbest)
#Running the model for plotting
PO4_high_res = arange(0,4,0.001)
Mu = function(aPbest,Ynbest,PO4_high_res)
D = Mu
...
#Unite conversion
#==================================
Numbertoarray=ones(size(PO4_high_res))            #(dimensionless) Number to array converter
#----------------------------------
#For N/C
#----------------------------------
...
Cnstore_plot=Cnstore/Qc*percentorratio*Numbertoarray
...
figure(1)
plot(PO4,Mu_d,'o',markeredgecolor='#882255',color='#882255')
plot(PO4_high_res,D*86400,linewidth=4,color='black')
xlabel('PO$_{4}^{-}$ ($\mu$M)')
xlim(xmax=3)
...
StackPlotColorsP2=(Color_Other,Color_Photo,Color_Bio,Color_Nstore,Color_other)


convert_mole=3.87e-4

Pconst_other_plot=Pconst_other*Punit*Numbertoarray* convert_mole      #(ug P/ mgC) PHosphorus in other parts (ex. phospholipid in outer membrane, ATP, ADP, etc. (assuming constant) (193-33)
Pthylakoid_plot=Pthylakoid*Punit*Numbertoarray* convert_mole     #(ug P/ mgC) Phosphorus in thylakoid membranes: phospholipid, etc. (193-26)(193-33)
Pdna_const_plot=Pdna_const*Punit*Numbertoarray * convert_mole   #(ug P/ mgC) Phosphorus in constant part of DNA
Prna_plot=Prna*Punit*Numbertoarray * convert_mole   #(ug P/ mgC) Phosphorus in constant part of RNA
Pstore_plot= zeros(size(PO4))#Pstore*Punit    #(ug P/ mgC) Phosphorus in phosphorus storage
Pessential_plot=Pessential*Punit*Numbertoarray* convert_mole

# oleg=mpat.Patch(color='#0070C0', label="Other",alpha=0.75)
# pleg=mpat.Patch(color='#FFC000', label="Photosynthesis",alpha=0.75)
# bleg=mpat.Patch(color='#66FF66', label="RNA",alpha=0.75)
# 
# handles=[oleg,pleg,bleg]
figure(6)
#stackplot(PO4_high_res,Pconst_other_plot+Pdna_const_plot,Pthylakoid_plot,Prna_plot, colors=StackPlotColorsP2)
stackplot(PO4_high_res,Pconst_other_plot+Pdna_const_plot,Pthylakoid_plot,Prna_plot,colors=StackPlotColorsP2, alpha=0.5)
stackplot(PO4_high_res,Pconst_other_plot+Pdna_const_plot,Pthylakoid_plot,colors=StackPlotColorsP2)

#title("P:C")
xlabel('PO$_{4}^{3-}$ ($\mu$M)')
ylabel("P:C (mol mol$^{-1}$)")
ylim(top=0.008)
#legend(handles=handles,loc='center',bbox_to_anchor=(0.5,-0.25),ncol=3, fontsize='x-small',frameon=False)
sf('timm_1b_allocation')


# figure(2)
# stackplot(PO4_high_res,Nconst_protein_plot,Nphoto_plot,Nbiosynth_plot,Ndna_const_plot,Nrna_plot,Nchl_plot,colors=StackPlotColorsN)
# title("N:C")
# xlabel(Xlabel)
# ylabel("N:C ($\\mu$g N / mg C)")
# 
# figure(4)
# stackplot(PO4_high_res,Cessential_plot,Cconst_protein_plot,Cphoto_plot,Cbiosynth_plot,Cdna_const_plot,\
#           Crna,Cchl_plot,CthylakoidPG_plot,Cnstore_plot,Cother_plot,colors=StackPlotColorsC)
# xlabel(Xlabel, fontsize=30)                       #copied from 73
# ylabel('C allocation ($\%$)', fontsize=30)  #copied from 73
# 
# figure(5)
# plot(repeat_times,aP_array)
# ylabel('aP')
# xlabel('repeat time')
#  
# figure(6)
